...
More specifically, there's a `command`, `query`, and `ui` profile present, thus separating the `Game` aggregate, `GameCatalogProjector`, and `GameRentalRestController` into distinct groups.
